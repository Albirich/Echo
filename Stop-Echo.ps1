# Stop-Echo.ps1  â€” PowerShell 5.1-safe
[CmdletBinding()]
param([switch]$Force)
...
$pidFiles = @(
  ...
  (Join-Path $state 'warm-router.pid'),
  (Join-Path $state 'warm-chat.pid')
)
...
Write-Verbose "[StopEcho] Closing launcher window (â€œEcho Room (Start)â€)..."
